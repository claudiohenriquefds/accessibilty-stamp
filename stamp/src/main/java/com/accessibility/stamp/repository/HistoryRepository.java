...
public interface HistoryRepository extends JpaRepository<HistoryEntity, Long> {
    ...
    @Query(value = "SELECT  1 AS id, 1 AS score, 1 AS site_id, 1 AS status, history_data.average, concat('2021-', history_data.created_at, '-01') as created_at FROM (SELECT  AVG(score) AS average, DATE_FORMAT(created_at, '%m') AS created_at FROM history WHERE DATE_FORMAT(created_at, '%Y') = DATE_FORMAT(NOW(), '%Y') GROUP BY DATE_FORMAT(created_at, '%m')) AS history_data", nativeQuery = true)
